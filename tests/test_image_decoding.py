...
class ImageDecodingTests(unittest.TestCase):
    ...
    def test_large_png_bounded_decode_and_alpha_compositing(self):
        ...
        self.assertEqual(preprocessing['version'], 3)
        self.assertTrue(preprocessing['downsampled'])
        self.assertEqual(preprocessing['working_width'], WORKING_IMAGE_SIZE)
        transparency = result['measurements']['transparency']
        self.assertEqual(transparency['width'], WORKING_IMAGE_SIZE)
        self.assertEqual(transparency['translucent_fraction'], 1)
        self.assertEqual(transparency['visible_bounds'], [0, 0, preprocessing['working_width'], preprocessing['working_height']])
        metadata, preview = prepare_image(path)
        self.assertEqual(preview.getpixel((0, 0)), (255, 0, 0, 128))
